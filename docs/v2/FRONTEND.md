...
|---|---|---|
| **Workspace** | Org/document list, upload, sensitivity tier confirmation | Evolves the "Analyze" section — **shipped (Phase 7)**: `frontend`'s `/upload` → `/documents/[id]`, a persistent `SensitivityBadge` sourced from the real `GET /api/v2/documents/{id}/sensitivity` response |
...
| **Chat** | True multi-turn assistant backed by session memory (`AGENTS.md`), not V1's stateless per-message call | Evolves "Chatbot" widget — **not built**: `frontend`'s `DocumentActions` ask box is still single-turn, like V1 |
