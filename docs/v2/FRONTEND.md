...
V1's frontend is a single static HTML page with vanilla JS and no build step — appropriate for a 6-endpoint demo, insufficient for a system with streaming multi-agent traces, collaborative redlining, and a knowledge-graph explorer. V2 replaced it with a proper SPA (`v2/`; the V1 `frontend/` has been removed) while keeping the same "ship something a browser can render with no exotic runtime requirements" spirit.
...
|---|---|---|
| **Workspace** | Org/document list, upload, sensitivity tier confirmation | Evolves the "Analyze" section — **shipped (Phase 7)**: `v2`'s `/upload` → `/documents/[id]`, a persistent `SensitivityBadge` sourced from the real `GET /api/v2/documents/{id}/sensitivity` response |
...
| **Risk Dashboard** | Spider/radar chart of risk categories, drill-down to clause-level explanations with counterfactual attribution | Closes the gap flagged in V1's `FEATURES.md` (README promised, never built) — **baseline shipped (Phase 8, `LEARNING_LOG.md` #52)**: `risk_radar/rules.py::RISK_CATEGORIES` gives the 55 risky terms an 8-category taxonomy; `POST /api/v2/documents/{id}/risk-dashboard` aggregates per-category counts across the document; `RiskDashboardPanel` renders a hand-rolled SVG radar chart with click-to-drill-down into the clause-level flags. Counterfactual attribution (this row's other half) is `NOVELTY.md` #4's still-blocked mechanism, needing redline history that doesn't exist. |
...
| **Knowledge Graph Explorer** | Visual traversal of entities/obligations/contradictions across a user's document portfolio | New — **text version (Phase 7) + visual graph (Phase 8) both shipped**: `KnowledgeGraphPanel` (ingest + term search over `/api/kg/{ingest,query,conflicts}`) plus `KnowledgeGraphExplorer` (`LEARNING_LOG.md` #53) — a real Cytoscape.js force-directed graph over `GET /api/kg/documents/{id}/graph`'s node/edge export, type-colored, with portfolio-linked terms (cross-document `SAME_AS`) highlighted and click-to-inspect. Obligation-level traversal stays out of scope until `Obligation` nodes exist (`schema.py`'s documented gap) |
| **Chat** | True multi-turn assistant backed by session memory (`AGENTS.md`), not V1's stateless per-message call | Evolves "Chatbot" widget — **not built**: `v2`'s `DocumentActions` ask box is still single-turn, like V1 |
