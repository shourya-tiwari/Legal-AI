...
5. **Access control.** Org/role RBAC (Admin, Editor, Viewer) plus resource-level ABAC (a user reaches only their org's documents/sessions), enforced at the Gateway and re-checked at each internal service. **Role RBAC shipped (Phase 7, `LEARNING_LOG.md` #36), scoped to what actually exists**: `app/guard.py::require_role` gates `PUT /api/v2/documents/{id}/sensitivity` (admin-only) and `POST /api/review-queue/{id}/resolve` (admin/editor) on `ApiKey.role` (default `admin`, so every pre-existing key keeps full access). Resource-level ABAC (org-scoped document access) has been true since Phase 1 (`_load_doc` filters by `org_id`) — this closes the *role* half specifically. Honest limitation: the role attaches to an **API key**, not a logged-in user — there's no login/session system, so this is per-key RBAC, not the full per-user RBAC this item names. `AuditLog.actor_id` (new) attributes an audited action to the specific key, which is as far as attribution can go without real user identity.
