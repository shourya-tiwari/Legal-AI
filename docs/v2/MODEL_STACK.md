...
| Coreference resolution | **maverick-coref** (current SOTA-ish, lightweight) with **fastcoref** as the fast fallback | Contracts need long-range coref ("the Company" 40 pages later); maverick handles document-length context better than fastcoref while staying CPU-serveable. LLM-assisted coref via the Router is the escalation path for the hardest cases. **Not shipped** — `fastcoref`'s `biu-nlp/f-coref` checkpoint hits the transformers-5/torch-<2.6 `torch.load` CVE guard; pick a safetensors coref model or bump torch. |
