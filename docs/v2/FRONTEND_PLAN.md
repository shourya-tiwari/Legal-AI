...
| **M5** | Analysis surfaces | Risk Dashboard (radar/severity/heatmap/drill-down) · Negotiation Studio (diff/accept-reject/export) · Knowledge Graph Explorer (React Flow) · Planner & Agent Trace (React Flow DAG + timeline + verification + routing) · Timeline & Simulation |
| **M6** | AI Assistant | chat UI, markdown + Shiki, citations, doc-context selector, suggested prompts, conversation history (localStorage), copy/export/share, search, file upload → document |
| **M7** | Ops | Review Queue (rebuild) · Evaluation Dashboard (eval-runs + delta report + charts) · Model Router (rebuild: provider cards, latency, overrides) · Analytics |
| **M8** | Admin + Settings + Auth | Admin (health, users, models/overrides, egress log, feature flags, API-key guidance, routing-policy view) · Settings (org flags, webhook, negotiation preferences, appearance, shortcuts) · Profile · `/login` |
| **M9** | Polish | skeleton/empty/error states everywhere, keyboard shortcuts + context menus, micro-interactions, responsive + a11y audit, perf (lazy heavy libs, bundle check, Lighthouse), `CHANGELOG.md`, screenshots, final docs sync |
