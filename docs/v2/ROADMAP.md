...
The rest of the phase — packaging, collapsed data layer, durable execution, Memory Service, per-user RBAC, the PII redaction gate — is **explicitly blocked on infrastructure this environment doesn't have**, not merely deferred by choice: there is no Kubernetes cluster, container registry, or air-gapped network segment to build and validate against here. Writing Zarf/OpenTofu/Harbor/cosign manifests without a target to run them against would produce unverified configuration — worse than no configuration, because it looks done. Revisit this whole block once a real deployment target exists.
