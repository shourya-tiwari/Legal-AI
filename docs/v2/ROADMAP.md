...
**Frontend SPA** (deferred from Phase 4 — a large workstream) — 🟡 first slice shipped, `LEARNING_LOG.md` #27
...
- [x] Workspace + Document Analyzer (V1 parity) — upload (`POST /api/upload`, still V1's endpoint) redirects into `/documents/[id]`: a left-panel clause list with **per-clause** rewrite/risk-scan/contextualize via `block_id` (something V1's whole-blob UI structurally could not offer), whole-document rewrite/timeline/risk-scan/ask for V1 parity, and a bonus "run full agent analysis" panel surfacing the planner's `plan`/`plan_rationale`, risk findings, KG conflicts, and faithfulness result — real backend capability (`app/agents/`) that had no UI anywhere before this. V1's `frontend/` is untouched and keeps serving.
- [ ] Agent Trace Viewer (real-time via session WebSocket), human-in-the-loop review queue UI — still not started, need the session WebSocket infrastructure this phase's Memory Service section also hasn't built.
...
**Verification**: `npm run build` + `npm run lint` clean; every endpoint exercised live via `curl` against the real backend with the exact request/response shapes the TypeScript client sends/expects (upload → rewrite/map/risk-scan/ask/contextualize/analyze on document id 3) — Gemini was intermittently returning `503 UNAVAILABLE` during this session, so a couple of calls needed a retry, but every one succeeded once Gemini cooperated, confirming the wiring itself is correct. **Not yet done**: an actual in-browser click-through — no `claude-in-chrome` extension was connected this session, so the UI has not been visually exercised, only server-rendered (curl'd `/` and `/documents/3`, both 200, no console/server errors) and API-verified.
