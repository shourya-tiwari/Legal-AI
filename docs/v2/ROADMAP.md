...
## Phase 4 — Agentic Orchestration MVP (~8-12 sprints) — ✅ Complete (backend MVP; frontend/Temporal/memory not started)
**GPU scope note**: nothing here needs a GPU — LangGraph is orchestration, and agents call Gemini or the existing CPU-only NLP/KG/RAG services. The one exception is the Verifier agent's NLI faithfulness check (a cross-encoder entailment model) — that specific sub-piece moves to Phase 7; the citation check and KG consistency check don't need it and ship now, using a lexical-overlap heuristic as the honestly-labeled interim faithfulness signal.
- LangGraph orchestration layer deployed (`AGENTS.md`); Extraction, Risk & Compliance, Clause Research, Summary, and Verifier agents implemented as a fixed pipeline (the core loop needed for a single-document analysis with citation-checked output) — **not yet done**: a dynamic Orchestrator/Planner agent (the pipeline is a fixed sequence, not a planner choosing what to run) and Temporal-backed durable execution (runs synchronously in-request instead; see `app/agents/graph.py`'s docstring for why that's an acceptable tradeoff at current workflow duration/cost, not a permanent decision).
- Memory Service — **not started**. Not GPU-related; genuinely separate scope not yet picked up.
- Frontend Agent Trace Viewer — **not started**. The whole project has been backend-only through Phase 4; the vanilla-JS frontend from V1 is unchanged. Not GPU-related — a real, sizable remaining workstream whenever frontend work is prioritized.
- `/api/v2/*` endpoints — **not introduced**; new capabilities keep shipping as new endpoints on the existing unversioned `/api/*` surface (e.g. `/api/agents/analyze`) since nothing has needed to *change* an existing V1 response shape yet.
- Human-in-the-loop review queue — **partially done**: every analysis run produces a `needs_human_review` flag (set when a KG conflict is found, a citation is invalid, or the faithfulness heuristic fails) and the full trace is queryable via `agent_traces`, but there's no actual queue/inbox UI or workflow for a human to act on it yet — the signal exists, the review mechanism doesn't.

**What's real and verified live** (against actual Postgres/Redis/Memgraph, not just mocked tests): uploading a document, ingesting it into the knowledge graph, then running `/api/agents/analyze` produces keyword risk flags, cross-document KG conflict candidates, targeted RAG citations, a Gemini-generated summary that correctly cites those sources by number, and a verifier that correctly flags the run for human review when a cross-document conflict is present — with every step's input/output persisted to `agent_traces` in Postgres.

**Exit criteria**: a full single-document analysis (extract → risk → research → verify) runs as an auditable agent workflow, with every claim citation-checked before reaching the UI. — met at the backend level (there is no UI yet to speak of "before reaching" — that's Phase 4's frontend gap, not a met-differently criterion).

## Phase 5 — Portfolio Intelligence & Research Features (~10-14 sprints, research-gated)
**GPU scope note**: less GPU-dependent than it first looks. The Risk Scoring Model (LightGBM) trains fine on CPU — it's blocked on labeled data, not hardware, and was previously mis-filed under the GPU Upgrade phase (corrected in `docs/v2/TASKS.md`). The Clause/Contract Type Classifier's fine-tuned-transformer version is genuinely GPU-work (Phase 7); the Document Sensitivity Classifier and the Redline Acceptance Predictor should each get a classical-ML attempt first, since neither obviously requires a transformer. Of `NOVELTY.md`'s ideas, only #1 and #3 have a GPU-bound training step — the other three (and the research/design work behind #1 and #3) don't need one.
...
- Agents: the Verifier agent's real NLI faithfulness check (Phase 4 ships a lexical-overlap stand-in first).
- Research track: the GNN training step for `NOVELTY.md` Idea #1 and the contrastive embedding training step for Idea #3 — everything else in the research track (including the design/prior-art work behind these two) is CPU-only and doesn't wait on this phase.
