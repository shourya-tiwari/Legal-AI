...
- **per-key RBAC** (`ARCHITECTURE.md` §5, `LEARNING_LOG.md` #36) — `app/guard.py::require_role`: `ApiKey.role` (`admin`/`editor`/`viewer`, default `admin` so every key issued before this shipped keeps full access) is checked on the two actions already documented as "no per-user RBAC yet": `PUT /api/v2/documents/{id}/sensitivity` (admin-only) and `POST /api/review-queue/{id}/resolve` (admin or editor). `AuditLog.actor_id` (new column) attributes every audited request to the specific key that made it.
- **per-user identity** (`ARCHITECTURE.md` §5, `LEARNING_LOG.md` #37) — closes the gap the per-key work above named as its own remaining limitation: `User`/`Session` tables, `app/auth.py` (PBKDF2-HMAC-SHA256 password hashing — stdlib only, no new dependency), `app/routes/auth.py` (`POST /api/auth/login`/`logout`, admin-only `POST`/`GET /api/auth/users`, `POST /api/auth/users/{id}/revoke`). `get_current_org` resolves either credential kind (an `lai_`-prefixed API key or a `sess_`-prefixed session token) to the same `OrgContext`; `AuditLog.actor_type` (`"api_key"`|`"user"`) disambiguates whose id is in `actor_id`, since both are plain ints from different tables. No self-serve signup or password reset — an org's first user is created by an existing admin or a bootstrap step, the same shape `create_api_key.py` already has for keys.
...
The rest of the phase — collapsed data layer, durable execution, Memory Service, and *deploying* the packaging below to a real target — is **explicitly blocked on infrastructure this environment doesn't have**, not merely deferred by choice: there is no Kubernetes cluster, container registry, or air-gapped network segment to build and validate against here. **RBAC is now fully shipped at both levels** (`LEARNING_LOG.md` #36/#37): per-key roles for machine credentials, and per-user identity (login/session, `User`/`Session` tables) for the case a key structurally couldn't cover — two people sharing one org no longer have to share one role or be indistinguishable in the audit trail. See the security bullets below.

**Build & supply chain** — 🟡 re-audited (`LEARNING_LOG.md` #38): Docker and kubectl are actually installed and working in this environment, contrary to the previous blanket "no infra at all" framing — everything checkable without a live cluster/registry is now written and, where checkable, checked. `helm`/`terraform`/`tofu`/`zarf`/`kit` genuinely aren't installed, so their artifacts are written-not-verified.
- [x] **Produce on-prem/air-gapped builds with `legalai-providers-external` excluded; enforce with an SBOM allowlist** — `backend/Dockerfile` (`core`/`external` profiles), `scripts/check_sbom_allowlist.py`. **Actually built and run**: both Docker profiles built successfully; the `core` image confirmed to have no `google-genai` installed (not just unimported); the allowlist script passes the `core` image and correctly *fails* when checked against the `external` image under the `core` policy — a real negative control, not a script that vacuously passes anything.
- [x] **Zarf** packaging (`zarf.yaml`, repo root) — components for the backend image, bundled data layer, optional GPU inference stack, and seed corpus. Written against Zarf's documented schema; `zarf` isn't installed here, so `package create`/`deploy` are unrun.
- [x] Model weights as a **KitOps** `Kitfile` (repo root) — the self-hosted model set as one versioned artifact spec. Same caveat: no `kit` CLI here.
- [x] **OpenTofu**, Helm, Kustomize overlays per profile — `deploy/kustomize/{base,overlays/{on-prem,cloud,gpu}}` (**verified**: all four render via `kubectl kustomize`, including the on-prem secret patch and the gpu overlay's two additive `NetworkPolicy` objects), `deploy/helm/legalai/` (same three profiles + templated `DATABASE_URL` composition Kustomize structurally can't do — **not verified**, no `helm`), `infra/opentofu/` (applies the already-validated Kustomize overlay rather than re-modeling every resource in HCL — **not verified**, no `terraform`/`tofu`). No k3s cluster in this environment to target.
- [x] cosign signing + Syft SBOM + Trivy/Grype scanning — a new `sbom-and-image` CI job builds both image profiles, re-runs the SBOM allowlist check (including the negative control) exactly as verified locally, then Syft/Trivy via their standard GitHub Actions. cosign is wired but a no-op placeholder — signing needs a pushed image in a real registry, which doesn't exist for this project. The Syft/Trivy/cosign steps haven't been executed here (no CI runner in this session).
- [ ] Egress proxy / network policy for the hybrid profile: deny all outbound except configured provider endpoints — **manifests written and rendered** (`deploy/kustomize/overlays/{on-prem,cloud}/network-policy.yaml`), still `[ ]` because a `NetworkPolicy` is inert unless the cluster's CNI implements it, and there's no cluster here to confirm one does — this specific line's enforcement half is genuinely, not just conveniently, unverifiable without a real target.
...
See `deploy/README.md` for the precise "written vs. rendered vs. actually deployed" table across every artifact in this section.

**Collapsed data layer** (the "one fewer service to operate" profile) — 🟡 `LEARNING_LOG.md` #39: re-checked against the actual codebase rather than assumed blocked. **pgvector-as-Qdrant-substitute was moot** — this project never adopted Qdrant; dense retrieval has used an embedded, in-process FAISS index since Phase 3/5, already satisfying "no separate vector service" for every profile. **KùzuDB-as-Memgraph-substitute genuinely shipped and is tested against a real embedded database** (`app/services/kg/kuzu_client.py`, `Settings.KG_BACKEND="kuzu"`, `tests/test_kg_kuzu_client.py`) — no server, no Docker needed, just `pip install`. One real Cypher-dialect difference found and fixed empirically (Kuzu's binder rejects reusing a bound node object across `WITH`/`UNWIND` into a later `MATCH`; every other query in `builder.py`/`queries.py` is unchanged across both backends). LanceDB/filesystem object storage/a Postgres event queue remain unbuilt for reasons that aren't "blocked on infra" — see `TASKS.md` for the honest reason attached to each.
- [x] Support **KùzuDB** as a Memgraph substitute (Apache AGE not evaluated — Kuzu's embedded, zero-server model was the better fit for the actual "one fewer service" goal). Profile-selected via `Settings.KG_BACKEND`, not Helm values (no `helm` binary in this environment to wire that path, `deploy/helm/legalai/values.yaml` doesn't yet expose it — a real follow-up once Helm itself is verified).
- [x] The same application code runs against either data-layer profile — for the graph half: `write_document_graph`/`link_portfolio_terms`/`find_clauses_using_term`/`find_potential_conflicts`/`get_document_graph_summary` all pass against a real, temporary Kuzu database, not a mock (a stronger check than exists for the Memgraph path itself, which this suite only ever exercises via a `FakeKGClient`).

**Durable execution & Memory Service** (deferred from Phase 4) — 🟡 `LEARNING_LOG.md` #40: re-checked before assuming blocked. DBOS was already named as the default specifically *because* it needs no new cluster (a library, Postgres-backed) — the "wrong pick is expensive to unwind" caution applies to choosing between DBOS/Hatchet/Temporal for a specific customer's scale, not to whether the already-chosen default could be built and tested here.
- [x] **Durable-execution engine — DBOS**, tested against a real Postgres database (`app/services/durable/dbos_engine.py`). A genuinely separate orchestrator over `app/agents/graph.py`'s same `AGENT_REGISTRY`/planner/`CaseState` — not a wrapper around LangGraph's own opaque `.invoke()`, which has no mid-run checkpoint hook to attach to. Each agent node is an individually-checkpointed `@DBOS.step()`; LangGraph stays the default in-request engine, this is opt-in via `Settings.DURABLE_EXECUTION_ENABLED`, wired into `routes/agents.py`. **Verified for real**: `tests/test_dbos_engine.py` (a live Postgres-backed DBOS instance, new CI job with a Postgres service container) confirms the DBOS-driven and LangGraph-driven runs produce identical `plan`/`ran_steps`/risk-finding results for the same document — the actual engine-agnostic claim, proven rather than asserted. Hatchet/Temporal not evaluated (DBOS alone already meets "no new cluster," the goal this environment could act on). A from-scratch crash-and-resume simulation (hard-kill mid-workflow, confirm a second process resumes without re-running the completed step) was attempted against real Postgres and abandoned after an unrelated subprocess-timing issue made it unreproducible here in the time available — that specific guarantee rests on DBOS's own tested behavior, not re-derived from scratch in this session.
- [x] **Memory Service, all three tiers** (`LEARNING_LOG.md` #41) — `app/services/memory/`: `session.py` (Redis-backed, TTL-bound, verified against a real Redis container), `episodic.py` (reads the existing `CaseAnalysis` table rather than duplicating it — "what did we find last time" plus embedding-similarity search across past summaries, the same approach `services/consistency.py` already established), `consolidation.py` (recurring-risk-term promotion into a new `SemanticMemoryEntry`, with an **unconditional** privacy-tier gate — `privileged`-tier documents never contribute, no opt-in override built). Required persisting `CaseAnalysis.risk_findings` first, closing another instance of the "computed then discarded" gap this table itself was originally built to fix for `needs_human_review`. No scheduler runs consolidation automatically yet (no job-queue/cron exists in this codebase).
- [x] **Typed tool interfaces** (`LEARNING_LOG.md` #41) — `app/agents/tools.py`: `kg_query`/`kg_conflicts`/`vector_search`/`statute_lookup`/`date_math`/`clause_diff`/`request_human_approval`, each Pydantic-validated on input and output through one `call_tool()` entrypoint. Every tool wraps an already-shipped service except the two genuinely new, deterministic, stdlib-only ones (`date_math`, `clause_diff`) and `request_human_approval` (honestly non-blocking — this codebase has no synchronous human-in-the-loop mechanism, only the async review queue). The planner still picks agents from a registry, not these tools by name — this ships the validated calling layer a future tool-choosing planner would sit on top of.
...
- [x] **Provider & Model admin, backend complete** (`LEARNING_LOG.md` #42): the eval-scores half was already real (`GET /api/models/eval-runs`). Per-task Class C toggles now real too — `ModelPolicyOverride` table, `GET/PUT/DELETE /api/models/class-c-overrides{/task}` (admin-only to write), wired into `router.py::_resolve_chain` so a toggle can only *remove* a Class C candidate the static policy already permitted, never add one back or weaken the unconditional `confidential`/`privileged` gate. The delta report is now callable too — admin-only `POST /api/models/delta-report` (a POST: it makes real generation calls with real latency/cost, not a passive status read) wraps the already-existing `app/eval/delta_report.py`. No `/models` UI renders either yet — same "capability real, no UI" gap this doc already tracks elsewhere.
- [x] **Model status panel latency, backend complete** (`LEARNING_LOG.md` #42): `GET /api/models/status` now returns `recent_avg_latency_ms`/`recent_call_count` per provider, aggregated from `model_calls.latency_ms` — that data has been recorded on every routing decision since Phase 5, simply never aggregated for this endpoint. Queue depth has no equivalent to surface — no job queue exists anywhere in this codebase.
...
- [x] **Introduce `/api/v2/*` document-first endpoints, per-org feature-flagging included** (`LEARNING_LOG.md` #42) — **first slice shipped** (`app/routes/v2.py`: `GET /api/v2/documents/{id}` + `POST .../{analyze,rewrite,map,ask,risk-scan,contextualize,consistency,simulate}`, reusing the V1 services, `block_id` to target one block); **now consumed by a real client** (`frontend-v2`). `Organization.feature_flags` (a flag missing from an org's dict falls back to a hardcoded default) + `app/guard.py::require_feature` gates every `/api/v2/*` route on `api_v2_enabled` (default `true`, so no org is broken by this shipping); `GET`/admin-only `PUT /api/org/settings` manages it. Session objects, `/negotiate` are still to do.
- [x] **Notification/Webhook Service for async job completion** (`LEARNING_LOG.md` #42) — `Organization.webhook_url` + `app/services/org_settings.py`, fired via FastAPI `BackgroundTasks` from `routes/agents.py::run_and_persist_analysis` after the HTTP response is sent. Honestly scoped: analysis itself still runs synchronously (or via DBOS), there's no true background job queue in this codebase — a webhook fired right after a synchronous call is still a real, useful completion signal, just not a queued-and-delivered-later one.
...
- [x] **Document Sensitivity Classifier — classical (TF-IDF + linear) tried and eval-gated for real** (`LEARNING_LOG.md` #43): `backend/training/{prepare_sensitivity_data,train_sensitivity_classifier}.py`, trained on 300 synthetic documents (real clause snippets already in this repo, weak-labelled by the existing rule classifier — no real customer documents exist to train on). Scored against the 11 real, hand-labelled `SENSITIVITY_GOLD` examples (never seen in training): **0.818 vs. the rule baseline's 1.000 — did not pass the gate, not promoted**; full result and failure analysis (structured PII patterns and milder confidentiality phrasing are the two misses) in `backend/training/models/sensitivity_classifier_card.md`. The transformer fine-tune this line's own "only if classical underperforms" names is now honestly justified — and, like the other heads below, GPU-blocked.
- [ ] **Legal Clause Embedding Model** — contrastive fine-tune (`NOVELTY.md` #3) with hard-negative mining.
- [~] **MLflow** registry + **DVC** data versioning (both live now, genuinely local, no server) — MLflow logs every training run's params/metrics/artifacts to a local sqlite store (`backend/training/mlruns.db`); DVC (`dvc init` at repo root, local `dvc-storage/` remote) versions `backend/training/data/` and trained `.joblib` artifacts, keeping git holding only small pointer files. **Eval-gated model promotion wired into CI/CD is not built** — real, buildable follow-up, not infra-blocked. **Model cards** for every trained model — the sensitivity classifier above is the **first genuinely trained artifact this repo has ever had**, and it has a real card even though it wasn't promoted (a negative result is still worth documenting); the clause/deontic heads stay template-only, still GPU-blocked.
