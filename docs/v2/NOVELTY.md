...
**CPU-only prototype**: `backend/training/redline_attribution.py` (`LEARNING_LOG.md` #57) — the counterfactual fingerprint-delta attribution step: a redline is diffed into atomic legal changes (modal swap / negation / numeric shift / jurisdiction swap / other reword), and each change's share of the fingerprint movement is measured by reconstructing the counterfactual "after, with every op of that kind reverted" and re-embedding it, then compared against a background distribution of same-kind deltas. All single-change attribution scenarios pass (the change is known by construction). **Two things this prototype establishes as genuinely blocking, not just deferred**: (a) the "accepted vs. rejected" label needs *real redline history*, which does not exist anywhere in this app (`LEARNING_LOG.md` #51's audit) — the prototype's labels are *generated* from a hand-specified prior, with the same "model recovers the rule, not reality" ceiling `LEARNING_LOG.md` #43/#45 found; (b) the classical Redline Acceptance Predictor does not beat its majority baseline because unsigned fingerprint-delta *magnitude* cannot encode edit *direction* (`shall→may` vs `may→shall` have the same magnitude, opposite acceptance) — recovering direction needs the *signed* delta in idea #3's *learned* fingerprint subspace, making idea #4's dependency on idea #3 structural, not incidental. Full write-up: `backend/training/models/redline_attribution_notes.md`.
