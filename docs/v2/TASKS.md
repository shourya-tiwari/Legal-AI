...
**Done so far** (none needed the deployment machinery): the **dynamic Orchestrator/Planner** (`app/agents/planner.py`), the **`/api/v2` document-first API** (`app/routes/v2.py`, now including `consistency`/`simulate`), **document sensitivity tiering enforced end-to-end**, the **PII/PHI redaction gate** (`app/services/redaction.py`), the **frontend SPA**'s four shipped slices (V1 parity, NLP/CV/KG/Model-Router exposure, post-hoc Agent Trace Viewer, and this session's human-in-the-loop review queue + eval-scores admin view + self-hosted fonts). What's left in this phase — packaging, collapsed data layer, durable execution, Memory Service — is **explicitly blocked on infrastructure this environment doesn't have** (a Kubernetes cluster, a container registry, an air-gapped network segment), not deferred by choice; writing that config without a target to validate it against would produce unverified artifacts. **Per-user RBAC** is separately not-yet-built (a real gap, not an infra one — it needs a user/identity model that doesn't exist yet).
