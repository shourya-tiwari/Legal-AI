...
**Running now, no external anything**: FastAPI backend with 11 route modules; org-scoped auth (default-off) + rate limiting + audit log; Postgres/SQLite persistence with an interim column-migration shim; provider-agnostic **Model Router** (`generate`/`embed`/`rerank`/`entail`/`ner` capabilities, declarative policy, Class-A offline fallbacks for every task, import-linter contract); the full **NLP pipeline** (rule-based) + **CV triage**; **Knowledge Graph** + **hybrid RAG** (dense+sparse+graph, RRF, rerank); a **planner-driven agent pipeline** (extraction → planner → dispatch → verifier) with a **real Class-A NLI faithfulness head**; **GLiNER** zero-shot NER; **document sensitivity tiering** enforced end-to-end (confidential/privileged never leave the perimeter); a **graded eval harness** (LegalBench/MNLI) + **cutover gate with curated gold sets for `qa`/`clause_rewrite`/`timeline_extract`/`risk_analysis`** + `model_calls`/`eval_runs` telemetry; the **`/api/v2` document-first API** (first slice); a **fine-tuning scaffold** (not run); a **Next.js frontend** (`frontend-v2/`) that actually calls all of the above — V1-parity document actions, the agent-analysis panel, and (as of this session) the structured NLP analysis, knowledge-graph, and Model-Router-status views that previously had no UI anywhere. Test suite: **193 pass + 1 skip**.
...
**Not started**: on-prem packaging (Zarf/SBOM/Harbor), the collapsed data layer, durable execution, the Memory Service, the Agent Trace Viewer / human-in-the-loop review queue / Provider & Model admin (frontend, beyond the plain `/models` status page), Phase 8 portfolio features, and every `NOVELTY.md` research idea (none have a prototype yet — they're research proposals, not built features). All CPU/code — none need a GPU.
...
- [ ] Provider & Model admin: which models serve which tasks, the eval scores behind the policy, the delta report, per-task/tier Class C toggles — `/models` is a step toward the "which models" half; no eval scores, delta report, or toggles yet
