...
**Done so far** (none needed the deployment machinery): the **dynamic Orchestrator/Planner** (`app/agents/planner.py`), the **`/api/v2` document-first API** (`app/routes/v2.py`, now including `consistency`/`simulate`), **document sensitivity tiering enforced end-to-end**, **per-key RBAC** (`app/guard.py::require_role`, `LEARNING_LOG.md` #36), the **PII/PHI redaction gate** (`app/services/redaction.py`) and its paired **Class C egress audit trail** (`GET /api/audit/egress`, `LEARNING_LOG.md` #35), the **frontend SPA**'s four shipped slices (V1 parity, NLP/CV/KG/Model-Router exposure, post-hoc Agent Trace Viewer, and this session's human-in-the-loop review queue + eval-scores admin view + self-hosted fonts). What's left in this phase — packaging, collapsed data layer, durable execution, Memory Service — is **explicitly blocked on infrastructure this environment doesn't have** (a Kubernetes cluster, a container registry, an air-gapped network segment), not deferred by choice; writing that config without a target to validate it against would produce unverified artifacts. **Per-*user*** identity (distinct from per-*key*) is separately not-yet-built — it needs a real login/session system, a bigger unit of work than a role column.
...
- [ ] Per-*user* identity (distinct from the per-key RBAC above) — a real login/session system so two people sharing one org's key can be individually attributed and given different roles; a bigger unit of work than a role column
...
- [ ] Egress proxy / network policy for the hybrid profile: deny all outbound except configured provider endpoints — the network-enforcement half; genuinely needs a real egress boundary to build/validate against
