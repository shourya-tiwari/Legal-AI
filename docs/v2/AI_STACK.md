...
Sensitivity tiering is a document-level gate (all or nothing); it says nothing about incidental PII sitting inside a `public`/`internal` document that *is* allowed to reach Class C. **Shipped (Phase 7):** `app/services/redaction.py` closes that gap — a regex floor (SSN, credit card, email, phone) merged with optional GLiNER zero-shot spans (person names, physical addresses, via `ner_extract`) masks personal identifiers in the prompt the moment `Router.generate()` resolves a Class C provider, before that provider is dispatched. A self-hosted (Class B) call never passes through this check.
...
**Current state (Phase 6, partial):** dense (self-hosted — TEI/`bge-m3` on the GPU when the `gpu` compose profile is up, else in-process `sentence-transformers`, else the Class-A hashing embedder) + sparse (BM25) + GraphRAG hits fused via RRF, then reranked (TEI/`bge-reranker-v2-m3` → in-process cross-encoder → Class-A lexical). GraphRAG fusion is wired into the Clause Research agent; the Contextualizer route wiring is a follow-up. The Verifier's faithfulness step is now a **real Class-A NLI head** (`verify_nli` task, in-process DeBERTa-v3-MNLI, `providers/nli_local.py`), with the lexical-overlap heuristic kept only as the labelled fallback when the head isn't installed. LLM generation still routes to Gemini by default when `EXTERNAL_PROVIDERS_ENABLED` — the self-hosted cutover is gated by `app/eval/cutover_gate.py` and blocked on a served `local-llm` + a 24 GB+ GPU. Qdrant is not yet used (the corpus is small; FAISS in-process). See `ROADMAP.md`.
